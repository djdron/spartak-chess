...
namespace {
  ...
  void load_defaults(Options& o) {
    ...
    o["Book File"] = Option("res/book.bin");
  ...
  int OptionValueInt(const string& optionName)
  {
	  Options::const_iterator i = options.find(optionName);
      if(i == options.end())
          return 0;
      return FromString(i->second.currentValue);
  }
  string OptionValueStr(const string& optionName)
  {
	  Options::const_iterator i = options.find(optionName);
      if(i == options.end())
          return string();
      return i->second.currentValue;
  }
...
void init_uci_options() {
  ...
  thr.defaultValue = thr.currentValue = ToString(cpu_count());

  if (cpu_count() >= 8)
      msd.defaultValue = msd.currentValue = ToString(7);
}
...
void print_uci_options(UCI_Command& uci) {
  ...
  for (std::vector<Option>::const_iterator it = vec.begin(); it != vec.end(); ++it)
  {
      ...
      if (it->type == CHECK)
          uci << " default " << (it->defaultValue == "1" ? "true" : "false");
      else
          uci << " default " << it->defaultValue;

      if (it->type == SPIN)
          uci << " min " << it->minValue << " max " << it->maxValue;
...
bool get_option_value_bool(const string& optionName) {
  ...
  return OptionValueInt(optionName);
}
...
string get_option_value_string(const string& optionName) {
   ...
   return OptionValueStr(optionName);
}
...
void set_option_value(UCI_Command& uci, const string& name, const string& value) {
...
void push_button(UCI_Command& uci, const string& buttonName) {

  set_option_value(uci, buttonName, "true");
}
...
bool button_was_pressed(UCI_Command& uci, const string& buttonName) {

  if(!OptionValueInt(buttonName))
      return false;

  set_option_value(uci, buttonName, "false");
  return true;
}
...
namespace {
  ...
  : defaultValue(ToString(def)), currentValue(ToString(def)), type(t), idx(options.size()), minValue(0), maxValue(0) {}

  Option::Option(int def, int minv, int maxv)
  : defaultValue(ToString(def)), currentValue(ToString(def)), type(SPIN), idx(options.size()), minValue(minv), maxValue(maxv) {}
