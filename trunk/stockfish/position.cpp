...
void Position::print(UCI_Command& uci, Move m) const {
  ...
  if (m != MOVE_NONE)
  {
      ...
      uci << "Move is: " << col << move_to_san(p, m) << endl;
  }
  for (Rank rank = RANK_8; rank >= RANK_1; rank--)
  {
      uci << "+---+---+---+---+---+---+---+---+" << endl;
      for (File file = FILE_A; file <= FILE_H; file++)
      {
          ...
          uci << '|' << col << pieceLetters[piece] << col;
      }
      uci << '|' << endl;
  }
  uci << "+---+---+---+---+---+---+---+---+" << endl
...
bool Position::is_ok(int* failedStep) const {
  ...
  const bool debugBitboards = false;
  const bool debugKingCount = false;
  const bool debugKingCapture = false;
  const bool debugCheckerCount = false;
  const bool debugKey = false;
  const bool debugMaterialKey = false;
  const bool debugPawnKey = false;
  const bool debugIncrementalEval = false;
  const bool debugNonPawnMaterial = false;
  const bool debugPieceCounts = false;
  const bool debugPieceList = false;
  const bool debugCastleSquares = false;
